"""
    Interactive Long-Read Visualisation Tool (ILVT) is a software to visualize and identify rearrangements using long read alignment data.
    Copyright (C) 2022 Jacob Lewerentz
...
# <!-- Window for settings. The settings-Window is called inside main window and its' input-form is set via the main window (settings-window content is dynamic)-->
class inputSettings_window(QWidget):
    ...
    def executeWindow(self):
        ...
        try:    self.setWindowIcon(QIcon('logo.png'))
        except: pass
        #/
...
class genomeOverview_window(QMainWindow):
    ...
    def export_image(self):
        export_path = QFileDialog.getSaveFileName(self, 'Save file')
...
class ApplicationWindow(QMainWindow):
    ...
    ## MENU: FILE
    def import_bam(self):
        bam_path = QFileDialog.getOpenFileName(self, 'Alignment file import')
    ...
    def import_gtf(self):
        features_path = QFileDialog.getOpenFileName(self, 'Annotation file import')
    ...
    def import_repeats(self):
        masking_path = QFileDialog.getOpenFileName(self, 'Repeat annotation import')
    ...
    def import_vcf(self):
        vcf_path = QFileDialog.getOpenFileName(self, 'Structural variant import')
    ...
    def import_read_list(self):
        read_list_path = QFileDialog.getOpenFileName(self, 'Open file')
    ...
    def export_read_selection(self):
        read_list_path = QFileDialog.getSaveFileName(self, 'Save file')
        if not read_list_path: return
        read_list_path = read_list_path[0]
...
def main():
    # Set the working dir to script location, so the UI can be imported
